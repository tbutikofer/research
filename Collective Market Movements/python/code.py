...
def baseline_stress_synthetic_correlation(fit_data_files, trading_days_list, securities, simulation_runs, filename):
    ...
    popt1, pcov1 = curve_fit(prob_dist_T1, x_fit, y_fit[:,0], p0=(0.5, 0.3))
    ...
    return (popt1, popt2), (np.sqrt(np.diag(pcov1)), np.sqrt(np.diag(pcov2)))
...
def market_correlation(file_list):
    data_dict = {}
    for idx, filename in enumerate(file_list):
        data_csv = np.loadtxt('csv/'+filename+'.csv', dtype=np.unicode, delimiter=',')
        for data_record in data_csv[1:]:
#            date = data_record[0]
            date = datetime.strptime(data_record[0], base.DATE_FORMAT).date()
            value = data_record[1]
            if date not in data_dict:
                data_dict[date] = [None]*len(file_list)
            data_dict[date][idx] = value
    dates = np.array(list(data_dict.keys()))
    data = np.array(list(data_dict.values()))
    valid_idx = ~np.any(data == None, axis=1)
    data = data[valid_idx].astype(np.float)
    dates = dates[valid_idx]
#    data = data[~np.any(data == None, axis=1)].astype(np.float)
    return np.corrcoef(data, rowvar=False), (np.amin(dates).strftime(base.DATE_FORMAT), np.amax(dates).strftime(base.DATE_FORMAT))

...
#filename = code.simulate_random_stress(trading_days_list, securities, simulation_runs, (), change_range)
uniform_param_fit,  uniform_param_error= base.plot_baseline_stress(filename)
filename = 'market_norm_stress_normal'
#filename = code.simulate_random_stress(trading_days_list, securities, simulation_runs, ([20]), change_range)
normal_param_fit,  normal_param_error= base.plot_baseline_stress(filename)
\end{pycode}
...
\begin{align}
&f^0_\mathbb{T} \approx \alpha T^\beta\label{eqInstrinsicMarketStress}\\
\textrm{uniform distribution:}&\quad\alpha=\py{"{:0.2f}".format(uniform_param_fit[0])}\pm\py{"{:0.2f}".format(uniform_param_error[0])}\quad\beta=\py{"{:0.2f}".format(uniform_param_fit[1])}\pm\py{"{:0.2f}".format(uniform_param_error[1])}\nonumber\\
\textrm{normal distribution:}&\quad\alpha=\py{"{:0.2f}".format(normal_param_fit[0])}\pm\py{"{:0.2f}".format(normal_param_error[0])}\quad\beta=\py{"{:0.2f}".format(normal_param_fit[1])}\pm\py{"{:0.2f}".format(normal_param_error[1])}\nonumber.
\end{align}
\begin{figure}
\centering
...
\begin{pycode}
marketcode = 'SWX'
filename, date_range, sample_size = code.calc_change_distribution(marketcode, (-0.05,0.05), 2000, '31.12.2018')
fit_param = (80.0, 120.0, 3.3)
base.plot_histogram(filename, fit_param, r'$\Delta^{(SWX)}$')
\end{pycode}
\begin{figure}
\centering
	% trim={<left> <lower> <right> <upper>}
\pyc{print(r'\includegraphics[width=\columnwidth,trim={{40 40 50 50}},clip]{{{{"img/{}"}}}}'.format(filename))}
\caption{Probabilty distribution for \py{sample_size} quote changes on market \py{marketcode} during the period \py{date_range[0]} - \py{date_range[1]}.\\
The dashed curve indicates the approximation (\ref{eqQuoteChangeDist}) with paramters (\ref{eqSWXDist}).}
\label{figChangeDistSWX}
\end{figure}
\begin{equation}
p(\Delta)\approx e^{\alpha\frac{1-e^{\beta\Delta}}{1+e^{\beta\Delta}}\Delta+\gamma}\label{eqQuoteChangeDist}
\end{equation}
\begin{equation}
\alpha\approx\py{fit_param[0]}\quad\beta\approx\py{fit_param[1]}\quad\gamma\approx\py{fit_param[2]}\label{eqSWXDist}
\end{equation}
